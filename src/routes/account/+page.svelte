...
<div class="mt-28 flex w-full justify-center">
  <div class="w-full max-w-xs">
    ...
    {:else}
      {#if enterLoginToken}
        <form action="?/login" method="post" in:fade={{ delay: 200, duration: 200 }}>
          <input
            type="password"
            autofocus
      ...
      {:else}
        <button
          ...
          class="w-full rounded-lg bg-primary p-2 font-medium text-black duration-300 hover:opacity-75"
        >
          log in
        </button>
      {/if}
